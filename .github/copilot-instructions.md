...
- The top-level `docs/` folder contains numbered folders for major sections of the documentation.
- Each major section folder contains an `index.md` that defines the section and links to its subsections.
- Subsections are organized in subfolders with their own `index.md` files, following the same pattern recursively.
- The folder names use a numeric prefix to indicate their position in the hierarchy, followed by a descriptive slug.
- Images and other assets are stored in an `images/` subfolder within the relevant section folder.
